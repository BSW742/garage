...
/**
 * Somebody spun the wheel on a published site.
 *
...
// A wheel is a fun thing on a small business's page, not a mailing engine.
// These caps are what stop one bored visitor turning it into one.
const PER_IP_HOUR = 5;
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    let live = false;
    try { live = !!JSON.parse(String(row.config) || '{}')?.spinner?.on; } catch { /* no */ }
    if (!live) return json({ error: 'The wheel is not running on that site' }, 403);
    ...
    if (!overCap) {
      ...
      if (owner) {
        await sendMail(env, {
          ...
          replyTo: email,
          subject: won
            ? `${name} won "${prize}" on your wheel`
            : `${name} spun your wheel`,
          text:
            `${name} had a spin on ${slug}.garage.co.nz.\n\n` +
            (won ? `They landed on: ${prize}\n` : `They did not win anything this time.\n`) +
            `\nName:  ${name}\nEmail: ${email}\n` +
            (phone ? `Phone: ${phone}\n` : '') +
            `\nThey have not been emailed by us — the wheel says you will be in touch,\n` +
            `so if they won something, that part is yours.\n`,
        });
      }
    }

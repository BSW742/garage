...
/**
 * Somebody ran the digger on a published site.
 *
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    let live = false;
    try { live = !!JSON.parse(String(row.config) || '{}')?.dig?.on; } catch { /* no */ }
    if (!live) return json({ error: 'The digger is not running on that site' }, 403);
    ...
    if (!overCap) {
      ...
      if (owner) {
        await sendMail(env, {
          ...
          replyTo: email,
          subject: won
            ? `${name} dug up "${prize}" on The Big Dig`
            : `${name} hit the pipe on The Big Dig — ${prize}`,
          text:
            `${name} had a dig on ${slug}.garage.co.nz.\n\n` +
            (won ? `They knocked off holding: ${prize}\n` : `They hit the pipe and dropped to: ${prize}\n`) +
            `\nName:  ${name}\nEmail: ${email}\n` +
            (phone ? `Phone: ${phone}\n` : '') +

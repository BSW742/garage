...
/**
 * Somebody pumped the balloon on a published site.
 *
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    let live = false;
    try { live = !!JSON.parse(String(row.config) || '{}')?.balloon?.on; } catch { /* no */ }
    if (!live) return json({ error: 'The balloon is not running on that site' }, 403);
    ...
    if (!overCap) {
      ...
      if (owner) {
        await sendMail(env, {
          ...
          replyTo: email,
          subject: won
            ? `${name} tied off holding "${prize}" on The Balloon`
            : `${name} popped the balloon — ${prize}`,
          text:
            `${name} played The Balloon on ${slug}.garage.co.nz.\n\n` +
            (won ? `They tied it off holding: ${prize}\n` : `It popped and they dropped to: ${prize}\n`) +
            `\nName:  ${name}\nEmail: ${email}\n` +
            (phone ? `Phone: ${phone}\n` : '') +
    ...
    console.error('Balloon failed:', error);

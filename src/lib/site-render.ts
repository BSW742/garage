...
import { YOUTUBE_CSS, YOUTUBE_FONT_QUERY, renderYouTubeBody } from './youtube-render';
import { renderInstaBody, INSTA_CSS, INSTA_FONT_QUERY, type InstaPost } from './insta-render';
import { renderVideoMapBody, VIDEOMAP_CSS, VIDEOMAP_FONT_QUERY } from './videomap-render';
...
import { FITSTOP_CSS, FITSTOP_FONT_QUERY, renderFitstopBody } from './fitstop-render';
 ...
 * The waitlist.
 *
 * For a business that is booked out — "we have nothing until November, shall I
 * put you on the cancellation list?" — which today means a name on a pad and
 * somebody ringing down it when a slot drops.
 *
 * Nothing is discounted. The people on this list already wanted that slot at
 * full price and could not have it, so filling a cancellation from here is
 * recovering a booking that was otherwise lost rather than selling one cheap.
 */
...
export interface SiteConfig {
  ...
  style?: 'modern' | 'brutal' | 'classic' | 'cafe' | 'physio' | 'trade' | 'tribute' | 'listing' | 'diet' | 'chain' | 'bubbles' | 'game' | 'eggs' | 'mogged' | 'montage' | 'beauty' | 'yoga' | 'pilates' | 'workshop' | 'sauna'
    | 'rugby' | 'soccer' | 'basketball' | 'charity' | 'townhall' | 'daycare' | 'youtube' | 'insta' | 'videomap'
    // Off the shelf: asked for by name in the builder, never in the picker.
    | 'fitstop';
  ...
  balloon?: Balloon;   // The Balloon: pump it up or tie it off and keep it
  ringtoss?: RingToss; // Water ring toss: pure skill, rings on pegs, on the clock
  theme?: Theme;       // their own design language, read off their real site
  waitlist?: Waitlist; // the cancellation list: tell people when a slot frees up
}
...
export function renderSite(
  site: SiteConfig,
  slug: string,
  sent: TributePhoto[] | DietPost[] | ChainNote[] | InstaPost[] = [],
  state: { unlocked?: boolean; canAdd?: boolean } = {}
): string {
  ...
  if (site.style === 'youtube') {
    ...
    return shell(room, slug, {
      ...
      path: '/',
      body: renderYouTubeBody(room, slug),
    });
  }

  // A trip, told on the map it happened on. Stops in the order they were
  // visited, joined by a dashed line, each carrying a photograph or a film.
  if (site.style === 'videomap') {
    const trip: SiteConfig = {
      ...site,
      shop: false,
      tone: 'dark',
      palette: { primary: '#38bdf8', deep: '#0a0b0d', wash: '#141519', ...(site.palette || {}) },
    };
    const name = trip.name || slug;
    return shell(trip, slug, {
      title: name,
      description: (trip.lede || trip.headline || name).slice(0, 155),
      path: '/',
      body: renderVideoMapBody(trip, slug),
    });
  }

  // A wall of someone's own Instagram posts, packed tight. Instagram's embed
  // is the only sanctioned way to show a post and it is heavy, so the tiles
  // fill as you reach them rather than on load.
  if (site.style === 'insta') {
    const wall: SiteConfig = {
      ...site,
      shop: false,
      tone: 'dark',
      palette: { primary: '#e1306c', deep: '#0a0a0b', wash: '#141416', ...(site.palette || {}) },
    };
    const name = wall.name || slug;
    return shell(wall, slug, {
      title: name,
      description: (wall.lede || wall.headline || name).slice(0, 155),
      path: '/',
      body: renderInstaBody(wall, slug, (sent || []) as InstaPost[], { canAdd: state.canAdd }),
    });
  }
  ...
  // A gym. Not in the starter picker — reached by asking for it by name.
  if (site.style === 'fitstop') {
    const gym: SiteConfig = {
      ...site,
      chat: false,
      shop: false,
      tone: 'light',
      palette: { primary: '#f15922', deep: '#212532', wash: '#f2f2f0', ...(site.palette || {}) },
    };
    const room = gym.name || slug;
    return shell(gym, slug, {
      title: room,
      description: (gym.lede || gym.headline || room).slice(0, 155),
      path: '/',
      body: renderFitstopBody(gym, slug),
    });
  }
...
function shell(site: SiteConfig, slug: string, page: PageMeta): string {
  ...
  const extraFonts =
    ...
    : site.style === 'fitstop' ? FITSTOP_FONT_QUERY
    ...
    : site.style === 'youtube' ? YOUTUBE_FONT_QUERY
    : site.style === 'insta' ? INSTA_FONT_QUERY
    : site.style === 'videomap' ? VIDEOMAP_FONT_QUERY
